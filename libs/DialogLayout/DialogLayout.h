...
///////////////////////////////////////////////////////////////////////////////
// CLayoutRuleControlSize

class CLayoutRuleControlSize : public CLayoutRule
{
private:
	UINT m_controlId;

public:
	CLayoutRuleControlSize(UINT controlId)
...
///////////////////////////////////////////////////////////////////////////////
// CCenteredControlLayout

class CCenteredControlLayout : public CLayoutNode
{
private:
	UINT m_nID;
	UINT m_nAnchor;
...
public:
	CCenteredControlLayout(const CDialogTemplate&, const CLayoutContainer& parent, UINT controlID, UINT nAnchor);

	void DoLayout(CWindow wndLayout, HDWP hDefer, const CRect& rcLayout);
};
...
namespace DialogLayoutHelpers {
	template<class T>
	class LayoutBuilderData;
}
...
private:
	friend class DialogLayoutHelpers::LayoutBuilderData<T>;
...
protected:
	...
	inline CLayoutContainer *GetRootContainer() {
		return m_pRootContainer;
	}

	inline void SetRootContainer(CLayoutContainer *newRoot) {
		ATLASSERT(newRoot != nullptr);
		if (m_pRootContainer != nullptr) delete m_pRootContainer;
		m_pRootContainer = newRoot;
	}
...
{
	template<typename T>
	class LayoutBuilderData final
	{
	private:
		CDialogLayout<T> *m_Owner;
		CAtlList<CLayoutContainer *> m_ContainerStack;

	public:
		LayoutBuilderData(CDialogLayout<T> *owner) : m_Owner(owner) {
			m_ContainerStack.AddTail(m_Owner->GetRootContainer());
		}

		inline CDialogTemplate GetTemplate() const { return m_Owner->m_Tmpl; }
		inline CLayoutContainer *PeekContainer() const { return m_ContainerStack.GetTail(); }
		inline void PushContainer(CLayoutContainer *container) { m_ContainerStack.AddTail(container); }
		inline CLayoutContainer *PopContainer(void) { ATLASSERT(m_ContainerStack.GetCount() > 1); return m_ContainerStack.RemoveTail(); }
	};

	inline CLayoutRule *absolute(LONG coord)
	{
		return new CLayoutRuleAbsolute(coord);
	}

	inline CLayoutRule *control_size(UINT controlId)
	{
		return new CLayoutRuleControlSize(controlId);
	}

	inline CLayoutRule *left_of(UINT controlId, UINT padding = 0)
	{
		return new CLayoutRuleRelativeControl(controlId, LAYOUT_DIRECTION_LEFT, padding);
	}

	inline CLayoutRule *right_of(UINT controlId, UINT padding = 0)
	{
		return new CLayoutRuleRelativeControl(controlId, LAYOUT_DIRECTION_RIGHT, padding);
	}

	inline CLayoutRule *above(UINT controlId, UINT padding = 0)
	{
		return new CLayoutRuleRelativeControl(controlId, LAYOUT_DIRECTION_TOP, padding);
	}

	inline CLayoutRule *below(UINT controlId, UINT padding = 0)
	{
		return new CLayoutRuleRelativeControl(controlId, LAYOUT_DIRECTION_BOTTOM, padding);
	}

	inline CLayoutRule *ratio(double ratio)
	{
		return new CLayoutRuleRatio((float)ratio);
	}

	template<typename T>
	inline void layout_control(LayoutBuilderData<T>& vars, UINT controlId, UINT anchor)
	{
		CLayoutContainer *container = vars.PeekContainer();
		container->AddLayoutNode(new CLayoutControl(vars.GetTemplate(), *container, controlId, anchor));
	}

	template<typename T>
	inline void center_control(LayoutBuilderData<T>& vars, UINT controlId, UINT anchor)
	{
		CLayoutContainer *container = vars.PeekContainer();
		CCenteredControlLayout *layout = new CCenteredControlLayout(vars.GetTemplate(), *container, controlId, anchor);
		container->AddLayoutNode(layout);
	}

	template<typename T>
	inline void begin_layout_container(LayoutBuilderData<T>& vars, CLayoutRule *left, CLayoutRule *top, CLayoutRule *right, CLayoutRule *bottom)
	{
		CLayoutContainer *newContainer = new CLayoutContainer(vars.GetTemplate(), vars.PeekContainer(), left, top, right, bottom);
		vars.PeekContainer()->AddLayoutNode(newContainer);
		vars.PushContainer(newContainer);
	}

	template<typename T>
	inline void end_layout_container(LayoutBuilderData<T>& vars)
	{
		vars.PopContainer();
	}
}
