...
LONG CLayoutRuleControlSize::Apply(const CDialogTemplate& tmpl, UINT direction, const CRect& rcLayout) const
{
	CDialogItemTemplate item = tmpl.FindControl(m_controlId);

	if (direction == LAYOUT_DIRECTION_LEFT) return rcLayout.left;
	else if (direction == LAYOUT_DIRECTION_TOP) return rcLayout.top;
	else if (direction == LAYOUT_DIRECTION_RIGHT) return rcLayout.left + item.GetWidth();
	else return rcLayout.top + item.GetHeight();
}

LONG CLayoutRuleControlSize::Apply(CWindow wndLayout, UINT direction, const CRect& rcLayout) const
{
	CWindow control = wndLayout.GetDlgItem(m_controlId);
	ATLASSERT(control.IsWindow());

	RECT controlRect; wndLayout.GetWindowRect(&controlRect);
	::MapWindowPoints(HWND_DESKTOP, control.m_hWnd, (LPPOINT)&controlRect, 2);

	if (direction == LAYOUT_DIRECTION_LEFT) return rcLayout.left;
	else if (direction == LAYOUT_DIRECTION_TOP) return rcLayout.top;
	else if (direction == LAYOUT_DIRECTION_RIGHT) return rcLayout.left + (controlRect.right - controlRect.left);
	else return rcLayout.top + (controlRect.bottom - controlRect.top);
}
...
CCenteredControlLayout::CCenteredControlLayout(const CDialogTemplate& tmpl, const CLayoutContainer& parent, UINT controlId, UINT anchor)
	: m_nID(controlId), m_nAnchor(anchor)
{
	ATLASSERT(m_nAnchor != LAYOUT_ANCHOR_NONE);
}

void CCenteredControlLayout::DoLayout(CWindow wndLayout, HDWP hDefer, const CRect& rcLayout)
{
	CRect rcControl;

	CWindow wndControl = wndLayout.GetDlgItem(m_nID);
	CRect rcOld; wndControl.GetWindowRect(&rcOld); wndLayout.ScreenToClient(rcOld);

	int width = rcOld.Width(), height = rcOld.Height();

	CRect rcNew;
	if ((m_nAnchor & LAYOUT_ANCHOR_HORIZONTAL) != 0) rcNew.left = (rcLayout.Width() - width) / 2;
	else rcNew.left = rcLayout.left;
	rcNew.right = rcNew.left + width;

	if ((m_nAnchor & LAYOUT_ANCHOR_VERTICAL) != 0) rcNew.top = (rcLayout.Height() - height) / 2;
	else rcNew.top = rcLayout.top;
	rcNew.bottom = rcNew.top + height;

	wndControl.DeferWindowPos(hDefer, nullptr, RECT_BREAK(rcNew), SWP_NOACTIVATE | SWP_NOZORDER);
}
