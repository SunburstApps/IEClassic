...
extern ULONG NTAPI vDbgPrintExWithPrefix(LPCSTR Prefix, ULONG ComponentId, ULONG Level, LPCSTR Format, va_list ap);
...
static char *get_temp_buffer( size_t size )
{
	...
	idx = _InterlockedExchangeAdd( &pos, 1 ) % (sizeof(list)/sizeof(list[0]));
